<script lang="ts">
    ...
    const secretId = preEmergencyAccessData.secretId;
    const emergencyContactId = preEmergencyAccessData.emergencyContactId;
    const temporalSessionId = preEmergencyAccessData.temporalSessionId;
    const password = preEmergencyAccessData.password;
    const vShare = preEmergencyAccessData.vShare;
    const dataEncryptionKey = preEmergencyAccessData.dataEncryptionKey; 
    ...
    async function send2fa() {
      const result = await invoke("send_2fa_emergency_access_secret_code", { oneTimeSecretCode, secretId, emergencyContactId, temporalSessionId, password, vShare, dataEncryptionKey }) as TransientSecret;
       if (result) {
          transientSecret = result;
          preEmergencyAccessData.secretId = "";
          preEmergencyAccessData.emergencyContactId = "";
          preEmergencyAccessData.temporalSessionId = "";
        } else {
  ...
  <span>Warning: Copy all the information below in a safe place if you need to use it later.</span>
    ...
    {/if}
        ...
        {:else}
      <!-- Form -->
      <form class="space-y-4" onsubmit={send2fa}>
